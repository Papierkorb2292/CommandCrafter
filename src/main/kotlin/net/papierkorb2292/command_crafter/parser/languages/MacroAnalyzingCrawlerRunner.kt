 ...
 *
 * Another optimization is done by removing any nodes from crawlers that are known to never reach the literal count of the current best result. This is done using [AttemptLiteralCounter],
 * which can count how many child literals a node has and how many of those can be found in the input string.
 *
 * Also, there's a special handling for completions generated by 'trailing' spawners (= spawners starting after the last literal).
...
class MacroAnalyzingCrawlerRunner(
    ...
    fun run(): AnalyzingResult {
        ...
        while(weightedSpawners.isNotEmpty()) {
            ...
            if(bestMatchingSpawner.bestResult!!.hasNewLiteralNodes()) {
                val position = bestMatchingSpawner.bestResult!!.childSpawner?.startAttemptIndex ?: attemptPositions.size
                if(position > furthestLiteralPosition)
                    furthestLiteralPosition = position
            }
            ...
            for(spawner in mostPromisingSpawners) {
                ...
                if(spawner.startAttemptIndex >= furthestLiteralPosition)
                    trailingSpawnersPerAttemptIndex[spawner.startAttemptIndex] += spawner
            }
    ...
    private inner class Spawner(
        ...
        /**
         * Stores all crawler results from a crawler attempt at `attemptAnalyzingResults[attemptCount][skippedNodeCount]`
         */
        val attemptResults = mutableListOf<MutableList<List<CrawlerResult>?>>()
        ...
        fun runCrawlersOnce(spawnerIndex: Int) {
            ...
            while(i < crawlers.size) {
                ...
                while(attemptResults.size <= attemptCount) {
                    attemptResults.add(mutableListOf())
                }
                val attemptAnalyzingResultsForAttemptCount = attemptResults[attemptCount]
                ...
                attemptAnalyzingResultsForAttemptCount[crawler.skippedNodeCount] = results
    ...
    private inner class CrawlerResult(
        ...
        fun cutSpawnerTree() {
            ...
            if(literalNodeCount < CUT_OFF_MINIMUM_LITERAL_COUNT)
                return


            if(literalNodeCount - parentSpawner.baseResult!!.literalNodeCount >= CUT_OFF_MINIMUM_LITERAL_COUNT) {
                weightedSpawners.clear()
                if(childSpawner != null)
                    weightedSpawners += mutableListOf(childSpawner)
                return
            ...
            val lockedInLiterals = literalNodeCount - CUT_OFF_MINIMUM_LITERAL_COUNT
    ...
    private inner class AttemptLiteralCounter(private val nodeIdentifier: NodeIdentifier, private val nodeMaxLiteralCounter: NodeMaxLiteralCounter) {
        ...
        private fun copyIncrementedLiteralCount(positionIndex: Int, parentMap: Int2ByteLinkedOpenHashMap): Int2ByteLinkedOpenHashMap {
            ...
            val candidate = reader.string.substring(start, end)
            val candidateId = nodeIdentifier.tryGetIdForLiteral(candidate)
        ...
        fun markDirty(positionIndex: Int) {
            if(positionIndex > dirtyPositionMax)
                dirtyPositionMax = positionIndex
        }
    }
    ...
    class NodeIdentifier {
        ...
        /**
         * Returns the id for the literal, or -1 if the literal is not known
         */
