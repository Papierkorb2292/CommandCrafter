...
class CodecSuggestionWrapper<A, TContext>(private val delegate: Codec<A>, val contextGetter: ContextGetter<TContext>, val suggestionsProvider: ContextSuggestionsProvider<TContext>, val suggestEmptyString: Boolean): Codec<A> {
    companion object {
        ...
        fun <A> simple(delegate: Codec<A>, suggestionProvider: SuggestionsProvider, suggestEmptyString: Boolean): Codec<A> =
            CodecSuggestionWrapper(delegate, { _, _ -> }, object : ContextSuggestionsProvider<Unit> {
        ...
        fun <A, TContext> withThreadLocal(delegate: Codec<A>, threadLocal: ThreadLocal<TContext>, suggestionProvider: ContextSuggestionsProvider<TContext?>): Codec<A> =
            withContext(delegate, { _, _ -> threadLocal.getOrNull() }, suggestionProvider)

        fun <A, TContext> withContext(delegate: Codec<A>, contextGetter: ContextGetter<TContext>, suggestionProvider: ContextSuggestionsProvider<TContext>): Codec<A> =
            CodecSuggestionWrapper(delegate, contextGetter, suggestionProvider, false)
    }
    ...
    override fun <T: Any> decode(ops: DynamicOps<T>, input: T?): DataResult<Pair<A, T>> {
        ...
        val context = contextGetter.getContext(input, extraBehavior)
    ...
    fun interface ContextGetter<TContext> {
        fun getContext(node: Any, behavior: ExtraDecoderBehavior<*>): TContext
    }
}