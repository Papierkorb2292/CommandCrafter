...
interface DelegatingDynamicOps<T>: DynamicOps<T> {
    ...
    override fun getMapValues(input: T): DataResult<Stream<Pair<T, T>>> = delegate.getMapValues(input)
    override fun getMapEntries(input: T): DataResult<Consumer<BiConsumer<T, T>>> = delegate.getMapEntries(input)
    override fun createMap(map: Stream<Pair<T, T>>): T = delegate.createMap(map)
    override fun getMap(input: T): DataResult<MapLike<T>> = delegate.getMap(input)
    ...
    override fun get(input: T, key: String): DataResult<T> = delegate.get(input, key)
    override fun getGeneric(input: T, key: T): DataResult<T> = delegate.getGeneric(input, key)
